...
unsigned char xUSART1_putchar_n(unsigned char ch);
static char *outptr;
...
void xPrintf_Init(void)
{
  ...
  */
  ...
  xdev_out(xUSART1_putchar_n);
  xdev_in(xUSART1_getchar);  
  
  /* Output a message on Hyperterminal using printf function */
  //printf("\n\rUSART Printf Example: retarget the C library printf function to the USART\n\r");
  xprintf("\r\n\n\n\r WWW.ARMJISHU.COM  %s xprintf configured....", EVAL_COM1_STR);
...
unsigned char xUSART1_putchar_n(unsigned char ch)
{
  if('\n' == ch)
  {
    xUSART1_putchar('\r'); /*发送一个字符函数*/   
  }

  return xUSART1_putchar(ch); /*发送一个字符函数*/ 
}
